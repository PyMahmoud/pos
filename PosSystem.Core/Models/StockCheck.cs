...
{
    /// <summary>
    /// One "what's on the shelf today" reading from a single customer
    /// visit — quantity, batch/lot, and expiry, per the client's stated
    /// requirement. Append-only by design: never updated in place, only
    /// inserted, so the rep can see how a customer's stock of a given
    /// item trended visit to visit, not just the latest number.
    /// </summary>
