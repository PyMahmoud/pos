...
> **Later generalized:** the schema/UI built here (`stockchecks`, the sold-items
> summary, the item picker) was originally modeled on the pharma client's
> vocabulary (`MedicationName`, "Medications Sold", etc.) but has since been
> renamed to generic terms (`ItemName`, "Items Sold") so the POS works for any
> line of business, not just pharmacy distribution. The description below is
> left as the original historical record of the client requirement.

**What it is:** the client sells items to customers (his "customers") and visits them roughly daily to check what's left of what he's already sold them — a restock-decision signal, not his own warehouse stock (that's the existing Inventory screen's job). Confirmed directly with the client:
- It's the *customer's* remaining stock being recorded, not the rep's own
- Full history is needed (see how a customer's stock trends visit to visit), not just a latest-number overwrite
- Batch/lot number and expiry date are required per reading (pharma regulatory norm, kept as an available field for any business that needs it)

- [x] New `stockchecks` table (append-only — every visit is a new row, never updated in place): CustomerId, item, quantity, batch/lot, expiry, check date/time, notes
...
- [x] Customers screen: "View Details" on any customer card opens a drill-down (in-place, not a new sidebar item) showing:
  - **Items Sold** — aggregated from every Bills row linked to this customer (via the new `CustomerId`) joined to its Sells line items. This part needed zero new schema — it's the natural consequence of Checkout's existing customer picker (Phase 5) once sales are actually linked.
  - **Record a Stock Check** — a small form (item picker, quantity, batch/lot, expiry, notes) writing a new `stockchecks` row
...
**Exit criteria:** clicking into a customer shows what's been sold to them and lets the rep log a dated, batch/expiry-tracked stock reading from the field. *(Code is in place; needs the same real build/run pass every prior phase needed before calling it done.)*
